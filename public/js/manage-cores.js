...
class CoreManager {
    constructor() {
        ...
        this.coreColorMap = [
            '#0000ff', '#ff7f00', '#00ff00', '#964b00', '#808080', '#ffffff',
            '#ff0000', '#000000', '#ffff00', '#8f00ff', '#ff00ff', '#00ffff'
        ];
        this.init();
    }
    ...
    getCoreDataFromDOM(coreId) {
        ...
        const coreNumber = coreCard.querySelector('h4').textContent.replace('Core ', '');
        const attenuationEl = coreCard.querySelector('.space-y-2 .flex:nth-child(3) span:last-child');
    ...
    populateEditModal(core) {
        ...
        // Update modal header
        this.updateElement(document.getElementById('modal-core-title'), `Core ${core.core_number}`);
        this.updateElement(document.getElementById('modal-core-location'), `Tube ${core.tube_number} • Cable: ${core.cable.name}`);
    }
    ...
    async submitEdit(formData) {
        ...
        try {
            const response = await this.makeRequest(`/cores/${this.currentEditingCore}`, 'PUT', formData);
    ...
    updateStatusIndicators(coreCard, formData) {
        ...
        if (indicators[1]) {
            indicators[1].className = `w-3 h-3 rounded-full ${formData.status === 'ok' ? 'bg-green-500' : 'bg-red-500'}`;
            indicators[1].title = `Status: ${formData.status === 'ok' ? 'OK' : 'Not OK'}`;
        }

        // Usage indicator - check if exists
        if (indicators[2]) {
            indicators[2].className = `w-3 h-3 rounded-full ${formData.usage === 'active' ? 'bg-blue-500' : 'bg-gray-400'}`;
            indicators[2].title = `Usage: ${formData.usage.charAt(0).toUpperCase() + formData.usage.slice(1)}`;
        }
    }
    ...
    updateDetailSection(coreCard, formData) {
        ...
        const updates = {
            'Status:': {
                ...
                class: `font-medium ${formData.status === 'ok' ? 'text-green-600' : 'text-red-600'}`
            },
            'Usage:': {
                text: formData.usage.charAt(0).toUpperCase() + formData.usage.slice(1),
                class: `font-medium ${formData.usage === 'active' ? 'text-blue-600' : 'text-gray-600'}`
            },
    ...
    updateStatistics() {
        ...
        const stats = { total: allCards.length, active: 0, inactive: 0, problems: 0 };
        ...
        if (statsCards.length >= 4) {
            this.updateElement(statsCards[1].querySelector('.text-2xl.font-bold'), stats.active);
            this.updateElement(statsCards[2].querySelector('.text-2xl.font-bold'), stats.inactive);
            this.updateElement(statsCards[3].querySelector('.text-2xl.font-bold'), stats.problems);
        }
    ...
    applyFilters() {
        ...
        coreCards.forEach(card => {
            const shouldShow = (!filters.tube || card.dataset.tube === filters.tube) &&
    ...
    applyFiltersToCard(coreCard) {
        ...
        const shouldShow = (!filters.tube || coreCard.dataset.tube === filters.tube) &&
        ...
        if (tubeSection) {
            const visibleCards = tubeSection.querySelectorAll('.core-card[style="display: block"], .core-card:not([style*="display: none"])');
            tubeSection.style.display = visibleCards.length > 0 ? 'block' : 'none';
        }
    }
    ...
    async loadJointClosures() {
        try {
            const data = await this.makeRequest('/connections/joint-closures', 'GET');
    ...
    async disconnectCore(connectionId) {
        ...
        try {
            const data = await this.makeRequest(`/connections/${connectionId}`, 'DELETE');
            if (data.success) {
                location.reload();
            } else {
    ...
    showNotification(message, type = 'info') {
        ...
        notification.className = `fixed top-4 right-4 z-50 px-6 py-4 rounded-lg shadow-lg max-w-sm transform transition-all duration-300 translate-x-full ${colors[type] || colors.info}`;
    ...
    updateDescription(coreCard, description) {
        const existingDesc = coreCard.querySelector('.text-xs.text-gray-600.italic');
        const detailSection = coreCard.querySelector('.space-y-2');
    ...
    setupCascadeDropdowns() {
        ...
        if (jcSelect) {
            jcSelect.addEventListener("change", async () => {
                if (jcSelect.value) {
                    try {
                        const data = await this.makeRequest(`/connections/joint-closures/${jcSelect.value}/cables`);
        ...
        if (cableSelect) {
            cableSelect.addEventListener("change", async () => {
                if (cableSelect.value) {
                    try {
                        const data = await this.makeRequest(`/connections/cables/${cableSelect.value}/tubes`);
        ...
        if (tubeSelect) {
            tubeSelect.addEventListener("change", async () => {
                if (cableSelect.value && tubeSelect.value) {
                    try {
                        const data = await this.makeRequest(`/connections/cables/${cableSelect.value}/tubes/${tubeSelect.value}/cores`);
    ...
    initializeModals() {
        const modalContent = document.querySelector('#core-edit-modal .bg-white');
        if (modalContent) {
    ...
    setupFilters() {
        ...
        filterIds.forEach(id => {
            ...
            if (filters[id]) {
                filters[id].addEventListener("change", () => this.applyFilters());
            }
        });
    ...
    setupForms() {
        ...
        if (coreEditForm) {
            coreEditForm.addEventListener("submit", (e) => {
                ...
                const formData = {
                    ...
                    attenuation: document.getElementById('edit-core-attenuation').value || null,
                    description: document.getElementById('edit-core-description').value || null
                };
        ...
        if (joinForm) {
            joinForm.addEventListener("submit", async (e) => {
                ...
                try {
                    const data = await this.makeRequest("/connections", "POST", formData);
                    if (data.success) {
...
function openCoreEditModal(coreId) { coreManager.openEditModal(coreId); }
function closeCoreEditModal() { coreManager.closeModal('core-edit-modal'); }
...
function closeJoinModal() { coreManager.closeModal('join-core-modal'); }
function disconnectCore(connectionId) { coreManager.disconnectCore(connectionId); }
