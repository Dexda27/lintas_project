...
<div class="min-h-screen w-full overflow-x-hidden">
    ...
    <div class="mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
            <div class="min-w-0 flex-1">
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 truncate">List Cable</h1>
                <p class="text-gray-600 mt-2 text-sm sm:text-base">Manage cable data easily</p>
            </div>
            <div class="flex-shrink-0">
                <a href="{{ route('cables.create') }}"
                    class="bg-blue-600 text-white px-3 sm:px-4 py-2 rounded-md hover:bg-blue-700 transition-colors text-sm sm:text-base text-center inline-block w-full sm:w-auto">
    ...
    <!-- Cables Table -->
    <div class="bg-white rounded-lg shadow w-full min-w-0">
        ...
        <form method="GET" action="{{ route('cables.index') }}" class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row gap-2 w-full">
                ...
                <div class="flex gap-2 flex-shrink-0">
                    <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-3 sm:px-4 py-2 rounded-lg shadow transition-colors text-sm sm:text-base flex-1 sm:flex-initial whitespace-nowrap">
                        Search
                    </button>
                    @if(request('search'))
                    <a href="{{ route('cables.index') }}"
                        class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-3 sm:px-4 py-2 rounded-lg shadow transition-colors text-sm sm:text-base flex-1 sm:flex-initial text-center whitespace-nowrap">
                        Clear
                    </a>
                    @endif
                </div>
            ...
            <div class="border-b border-gray-200 p-4 last:border-b-0">
                ...
                <div class="mb-3">
                    <div class="flex flex-wrap gap-1 mb-1">
                        <span class="text-green-600 text-xs whitespace-nowrap">{{ $cable->active_cores_count }} Active</span>
                        <span class="text-gray-500 text-xs whitespace-nowrap">{{ $cable->inactive_cores_count }} Inactive</span>
                        @if($cable->problem_cores_count > 0)
                        <span class="text-red-600 text-xs whitespace-nowrap">{{ $cable->problem_cores_count }} Problems</span>
                        @endif
                    </div>
                ...
                <div class="flex justify-between items-center">
                    <div class="flex flex-wrap gap-1">
                        <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $cable->status === 'ok' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800' }} whitespace-nowrap">
                            {{ ucfirst($cable->status) }}
                        </span>
                        <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $cable->usage === 'active' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}} whitespace-nowrap">
                            {{ ucfirst($cable->usage) }}
                        </span>
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Cable ID</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Name</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Region</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Route</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Cores</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Status</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Usage</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Connection</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                <div class="max-w-xs truncate">{{ $cable->cable_id }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div class="max-w-xs truncate" title="{{ $cable->name }}">{{ $cable->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 whitespace-nowrap">
                                    {{ $cable->region }}
                            ...
                            <td class="px-6 py-4 text-sm text-gray-900">
                                <div class="max-w-xs break-words">{{ $cable->source_site }} → {{ $cable->destination_site }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div class="space-y-1">
                ...
                <div class="flex justify-center sm:justify-end">
                    <div class="flex items-center space-x-1">
                        ...
                        <div class="sm:hidden flex items-center">
                            <span class="relative inline-flex items-center px-3 py-1 text-xs font-medium text-white bg-blue-600 border border-blue-600 whitespace-nowrap">
